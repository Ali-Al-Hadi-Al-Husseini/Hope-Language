    ...
    def make_tokens(self):
        ...
        while self.current_char != None:
            ...
            if self.current_char in ' \t':
                ...
                _char = self.current_char
                self.advance()
                return [], IllegalCharError(start_pos, self.position,"'" + _char+ "'")
...
class String(Value):
    ...
    def change_to(self,_char, by_this):
        if isinstance(_char,String) and isinstance(by_this,String):
            for idx in range(len(self.value)):
                if self.value[idx] == _char.value:
                    self.value[idx] = by_this
            return self.value, None

        else:
            return None, Value.illegal_operation(_char,by_this) 
...
class Interpreter:
    ...
    def visit_ListacssesNode(self,node, context):
        ...
        if len(list_values) <= idx:
            return res.failure(Indexerror(node.start_pos,node.end_pos,"List index out of range"))
        return res.success(list_values[idx])


BuiltInFunction._print       = BuiltInFunction("print")
...
global_symbol_table.set("print", BuiltInFunction._print)
